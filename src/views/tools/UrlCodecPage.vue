  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <ion-header collapse="condense">
        <ion-toolbar>
          <ion-title size="large">URL Codec</ion-title>
        </ion-toolbar>
      </ion-header>
      ...
      <div class="codec-container">
        ...
        <ion-segment v-model="mode" class="mode-segment">
          <ion-segment-button value="component">
            <ion-label>组件模式</ion-label>
          </ion-segment-button>
          <ion-segment-button value="full">
            <ion-label>完整 URI</ion-label>
          </ion-segment-button>
        </ion-segment>
...
function handleDecode() {
  ...
  try {
    output.value = mode.value === 'component'
      ? decodeURIComponentSafe(input.value)
      : decodeURISafe(input.value);
  } catch (e) {
    error.value = '无效的编码输入';
    output.value = '';
  }
...
.mode-segment { margin-bottom: 4px; }
